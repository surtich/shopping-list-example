...
(function($) {
            ...
            model.resource.app = (function() {
                return {
                    getCategories: function(success, error) {
                        model.resource.load("/s/category", success, error);
                    },
                    getProducts: function(idCategory, success, error) {
                        if (idCategory == undefined) {
                            model.resource.load("/s/product", success, error);
                        } else {
                            model.resource.load("/s/product/" + idCategory, success, error);
                        }
                    },
                    ...
                    getLists: function(success, error) {
                        model.resource.load("/s/list/", success, error);
                    },
            
                    getList: function(success, error, idList) {
                        model.resource.load("/s/list/" + idList, success, error);
                    },
                    createList: function (success, error) {
                        model.resource.post("/s/list", {
                            "products":  JSON.stringify(_sanitizeProducts())
                        }, success, error);
                    },
            
                    updateList: function (success, error) {
                        model.resource.put("/s/list/" + model.shoppingList.getIdList(), {
                            "products":  JSON.stringify(_sanitizeProducts())
                        }, success, error);
                    },
            
                    removeList: function (idList, success, error) {
                        model.resource.del("/s/list/" + idList, success, error);
                    },
            
                    getProductsList: function(products, success, error) {
                        if (products) {
                            model.resource.post("/s/list/products",{
                    ...
                    isConected: function (success, error) {
                        model.resource.load("/s/login/is_conected", success, error);
                    }
                };
    ...
    model.ShoppingProduct = function (product) {
        ...
        if (product.hasOwnProperty("nameProduct")) {
